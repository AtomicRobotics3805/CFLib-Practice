 ...
 */
class MyTwoWheelOdometryLocalizer(val constants: MyTwoWheelOdometryConstants) : TwoTrackingWheelLocalizer(listOf(
    ...
    lateinit var perpendicularEncoder: MyEncoder
    lateinit var parallelEncoder: MyEncoder
    ...
    override fun initialize() {
        Constants.opMode.telemetry.addLine("4.1")
        Constants.opMode.telemetry.update()
        perpendicularEncoder = MyEncoder(
            Constants.opMode.hardwareMap.get(
                DcMotorEx::class.java,
                constants.PERPENDICULAR_NAME
            )
        )
        parallelEncoder = MyEncoder(
            Constants.opMode.hardwareMap.get(
                DcMotorEx::class.java,
                constants.PARALLEL_NAME
            )
        )
        Constants.opMode.telemetry.addLine("4.2")
        Constants.opMode.telemetry.update()

        if (constants.PERPENDICULAR_REVERSED) perpendicularEncoder.direction = MyEncoder.Direction.REVERSE
        if (constants.PARALLEL_REVERSED) parallelEncoder.direction = MyEncoder.Direction.REVERSE
        Constants.opMode.telemetry.addLine("4.3")
        Constants.opMode.telemetry.update()
...
interface MyTwoWheelOdometryConstants {
