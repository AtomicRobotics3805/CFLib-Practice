...
public class MyBNO055IMUUtil {
     ...
     */
    public static void swapThenFlipAxes(BNO055IMU imu, AxesOrder order, MyAxesSigns signs) {
        try {
    ...
    public static void remapZAxis(BNO055IMU imu, AxisDirection direction) {
        switch (direction) {
            case POS_X:
                swapThenFlipAxes(imu, AxesOrder.ZYX, MyAxesSigns.NPP);
                break;
            case NEG_X:
                swapThenFlipAxes(imu, AxesOrder.ZYX, MyAxesSigns.PPN);
                break;
            case POS_Y:
                swapThenFlipAxes(imu, AxesOrder.XZY, MyAxesSigns.PNP);
                break;
            case NEG_Y:
                swapThenFlipAxes(imu, AxesOrder.XZY, MyAxesSigns.PPN);
                break;
            case POS_Z:
                swapThenFlipAxes(imu, AxesOrder.XYZ, MyAxesSigns.PPP);
                break;
            case NEG_Z:
                swapThenFlipAxes(imu, AxesOrder.XYZ, MyAxesSigns.PNN);
                break;
        }
    }
     ...
     */
    @Deprecated
    public static void remapAxes(BNO055IMU imu, AxesOrder order, MyAxesSigns signs) {
        AxesOrder adjustedAxesOrder = order.reverse();
        int[] indices = order.indices();
        int axisSignValue = signs.bVal ^ (0b100 >> indices[0]);
        MyAxesSigns adjustedAxesSigns = MyAxesSigns.fromBinaryValue(axisSignValue);
